...
export class CrossLayerLinkService {
  ...
  async createInitialCrossLayerLinks(orgId: string, fileId: string, slots: string[]): Promise<number> {
    let linksCreated = 0
  ...
  private async getProjectScenes(orgId: string, fileId: string): Promise<any[]> {
    ...
    return result.records.map((r: any) => ({ id: r.get('id'), number: r.get('number'), title: r.get('title') }))
  }
  ...
  private async getProjectPurchaseOrders(orgId: string, fileId: string): Promise<any[]> {
    ...
    `
    const result = await this.neo4j.run(query, { fileId, orgId })
    return result.records.map((r: any) => ({ id: r.get('id'), number: r.get('number'), vendor: r.get('vendor') }))
  }

  private async createCrossLayerLink(fromEntityId: string, toEntityId: string, relationshipType: string, orgId: string): Promise<void> {
    const query = `
