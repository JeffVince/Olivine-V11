...
export class Classifier {
  ...
  constructor(postgres: PostgresService, neo4j: Neo4jService) {
    this.classificationService = new ClassificationService(postgres)
    this.taxonomyService = new TaxonomyService()
    this.postgres = postgres
    this.neo4j = neo4j
  }

  private async createSlotEdgeFact(fileId: string, slot: string, confidence: number, ruleId: string, orgId: string): Promise<void> {
    const query = `
  ...
  async classifyAndApply(params: {
  ...
  }): Promise<{ slotKey: string; confidence: number; method: string; ruleId?: string | null }> {
    ...
    const taxonomyClassification = {
      ...
      method: result.method === 'taxonomy' ? 'rule_based' as const : 'ml_based' as const,
  ...
  private calculateRuleConfidence(rule: any, fileMetadata: FileMetadata, resourcePath: string): number {
    let confidence = 0.7
    if (rule.mime_type === fileMetadata.mimeType) confidence += 0.2
    const fileName = path.basename(resourcePath).toLowerCase()
    if (rule.slot === 'SCRIPT_PRIMARY' && (fileName.includes('script') || fileName.endsWith('.fdx'))) confidence += 0.1
    if (rule.slot === 'BUDGET_MASTER' && fileMetadata.size > 100000) confidence += 0.05
